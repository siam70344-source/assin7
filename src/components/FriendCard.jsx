...
export default function FriendCard({ friend }) {
  ...
  // Status colors matching the Figma Design
  const statusClasses = {
  ...
  return (
    ...
    >
      <img src={friend.picture} className="w-20 h-20 rounded-full mx-auto border-4 border-gray-50 shadow-sm" alt={friend.name} />
      ...
      <div className="flex flex-wrap justify-center gap-1 mb-4">
        {friend.tags.map(tag => (
          <span key={tag} className="text-[10px] uppercase font-bold px-2 py-0.5 bg-gray-100 text-gray-500 rounded">
            {tag}
          </span>
        ))}
      </div>
